      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {animals.map(a => (
          <div key={a.id} className="bg-white rounded shadow p-4">
            <div className="text-sm text-gray-500 mb-2">{a.id}</div>
            <h3 className="text-lg font-semibold">{a.nome} <span className="text-gray-500">({a.especie} • {a.porte})</span></h3>
            <label className="block text-sm mt-2">Descrição</label>
            <textarea className="w-full border rounded px-3 py-2" value={a.descricao || ''} onChange={e => {
              const v = e.target.value;
              setAnimals(list => list.map(x => x.id === a.id ? { ...x, descricao: v } : x));
            }}/>
            <label className="mt-2 flex items-center gap-2">
              <input type="checkbox" checked={!!a.adotado} onChange={e => {
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={() => handleSave(a)} className="bg-indigo-600 text-white rounded px-3 py-2 hover:bg-indigo-700">Salvar</button>
              <button onClick={() => handleDelete(a.id)} className="bg-red-500 text-white rounded px-3 py-2 hover:bg-red-600">Deletar</button>
            </div>
          </div>
        ))}
      </div>
